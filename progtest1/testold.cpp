...
void ChartoBitstream(vector< BYTE> inVector, vector<bool> &outVector){
  for ( BYTE c : inVector){
    for(int i = 7; i >= 0; i--){
      bool k = (c >> i) & 1;
...
bool decompressFile ( const char * inFileName, const char * outFileName ){
  ...
  vector< BYTE> fileData = readFile(inFileName);
  if (fail == 1){
    //cout << "fail here" << endl;
    return false;
  }
  // loading char array to bit arrray
  vector<bool> bitStream = {};
  ChartoBitstream(fileData, bitStream); 
...
int main ( void )
{
  assert (  decompressFile ( "testfail6.bin", "tempfile" ) );
