...
class CSupermarket{
  public:
    ...
    CSupermarket& store(string item, CDate date, int amount){
      int expiration = date.To_seconds();
    ...
    bool check_existence(string &product){
    ...
    int dispense(string product, int amount){
    ...
    void sell(list<pair<string, int>> &shoppinglist){
      for (list<pair<string, int>>::iterator itr = shoppinglist.begin(); itr != shoppinglist.end(); itr++){
        if (storage.find((*itr).first) != storage.end()){
          int sold = dispense((*itr).first, (*itr).second);
          if (sold == (*itr).second){
      ...
      for (list<pair<string, int>>::iterator itr = shoppinglist.begin(); itr != shoppinglist.end(); itr++){
        ...
        if (check_existence(usable) == true){
...
int main ( void )
{
  ...
  list<pair<string,int> > l6 { { "Cake", 1 }, { "Coke", 1 }, { "cake", 1 }, { "coke", 1 }, { "cuke", 1 }, { "Cokes", 1 } };
   s . sell ( l6 );
  // for (list<pair<string, int>>::iterator itr = l6.begin();
  //  itr != l6.end(); ++itr){
